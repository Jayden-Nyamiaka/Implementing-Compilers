...
let passes =
  ["lvar"; "un"; "rc"; "ec"; "si"; "ah"; "pi"; "pc"; "pa"]

let passes_str = String.concat " " passes

let usage_msg =
  "compile <filename> " ^
    "[-pass <pass>] [-only] [-eval] [-sexp-width n] [-sexp-indent n] [-help]"
      ...
      | "ah" ->
        begin
          if !eval then
            ...
            read_sexp filename |> x86_var1_in |> ah
              |> x86_var2_out |> print_sexp
          else
            read_lvar filename |> un |> rc |> ec |> si |> ah
              |> x86_var2_out |> print_sexp
        end
      ...
      | "pi" ->
        begin
          if !eval then
            ...
            read_sexp filename |> x86_var2_in |> pi
              |> x86_int_out |> print_sexp
          else
            read_lvar filename |> un |> rc |> ec |> si |> ah |> pi
              |> x86_int_out |> print_sexp
        end
