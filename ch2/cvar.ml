...
let get_vars (CProgram (_, lts)) =
  ...
  let vars_in_exp = function
    | Atm a        -> vars_in_atm a
    | Negate a     -> vars_in_atm a
    | Add (a1, a2) -> add_vars (vars_in_atm a1) (vars_in_atm a2)
    | Sub (a1, a2) -> add_vars (vars_in_atm a1) (vars_in_atm a2)
    | _ -> []
  in
  ...
  in
    List.concat_map (fun (_, t) -> vars_in_tail t) lts
