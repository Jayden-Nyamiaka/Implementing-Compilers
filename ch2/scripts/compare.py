...
passes  = ['src', 'lvar', 'un', 'rc', 'ec', 'si', 'ah', 'pi', 'pc', 'pa']
...
def diff_filename(infilename):
    ...
    args = ['./compile', infilename, '-pass', outpass, '-only', '-no-fix-label']
    ...
    if stderr_data != '':
        print(stderr_data)
        raise Exception('compile: non-empty stderr data')
    ...
    args = ['diff', '-b', outfilename, reffilename]
    proc = p.Popen(args, text=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
    (stdout_data, stderr_data) = proc.communicate(timeout=timeout)
    ret = proc.returncode
    ...
    if stderr_data != '':
        print(stderr_data)
        raise Exception('diff: non-empty stderr data')
...
if __name__ == '__main__':
    ...
    if '-random' in args:
        ...
        try:
            ...
            fatal_error('`-random` argument must be a positive integer')
        except IndexError:
            fatal_error('no argument for `-random` option')
        if nrandom <= 0:
            fatal_error('`-random` argument must be a positive integer')
    ...
    filenames = list(filter(lambda s: not s.endswith('.pa'), orig_filenames))
    filenames = order_filenames(filenames)
    pa_filenames = list(filter(lambda s: s.endswith('.pa'), orig_filenames))

    if pa_filenames and not filenames:
        fatal_error("can't compare .pa files")
