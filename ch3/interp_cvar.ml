...
let rec interp_labelled_tails env lts : env_t * int =
  match lts with
    | [] -> failwith "nothing to interpret"
    | [(_, t)] -> interp_tail env t
    | (_, t) :: lts' ->
        let (env', _) = interp_tail env t in
          interp_labelled_tails env' lts'

let interp (CProgram (_, labelled_tails)) : int =
  (* Discard environment. *)
  let (_, i) = interp_labelled_tails Env.empty labelled_tails in i
