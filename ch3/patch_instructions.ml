...
let convert_block (block : X.binfo1 X.block) : block =
  let convert_arg (arg: X.arg) : arg = match arg with
    | X.Imm i -> Imm i
    | X.Reg r -> Reg r
    | X.Deref (r, i) -> Deref (r, i)
    | X.Var _ -> 
        failwith "patch instructions: var encountered, error in allocate registers"
  in
  let convert_instr (instr: X.instr) : instr list = match instr with
    ...
    | X.Jmp lbl -> [Jmp lbl]
  in
  let (X.Block (X.Binfo1, instr_lst) ) = block in
    let instr_lst' =
