...
let convert_sexp_to_loc s = 
  match s with
    ...
    | _ -> failwith "convert_sexp_to_loc: unhandled Imm case"

(* Build the interference graph. *)
let make_graph
      (g : LocUgraph.t) (lbs : (label * binfo2 block) list) : LocUgraph.t =
      let rec add_edge lst g curr = 
        match lst with
          | h :: t -> 
            if h <> curr then 
              let g' = LocUgraph.add_edge_new g curr h in
              add_edge t g' curr
            else add_edge t g curr
          | _ -> g
      in 
      let rec add_edge_comp_two g (lst : location list) s d = 
        match lst with
          | h :: t -> 
            if (h <> s && h <> d) then
              let g' = LocUgraph.add_edge_new g d h in
              add_edge_comp_two g' t s d
            else add_edge_comp_two g t s d
          | _ -> g
      in 
        ...
        match afters_lst with
          ...
          | afters :: tail ->
            ...
            match instr_lst with
              ...
              | Movzbq (a, b) :: t
              | Movq (a, b) :: t -> 
                let g' = add_edge_comp_two g afters' (convert_sexp_to_loc a) (convert_sexp_to_loc b) in
                instr_helper g' t initials tail
              | Callq (_, _) :: t ->
