...
(* Get a list of all the jump (including conditional) labels in a block. *)
(* Very similar to get_next_labels in uncover_live.ml *)
...
(* Makes a graph directed graph btwn labels and the labels they jump to, 
 * filtering out the "conclusion" label since it shouldn't be merged. *)
(* Very similar to order_labels in uncover_live.ml *)
let make_graph (lbs : (label * 'a block) list) : G.t =
 ...
 * This is inefficient but it will do for our purposes. *)
...
(* Merge two blocks. *)
let merge_blocks (Block (_, instrs1) : 'a block) 
                (Block (_, instrs2) : 'a block) : 'a block =
  let new_instrs = (butlast instrs1) @ instrs2 in
    Block (Binfo1, new_instrs)


(* Implements the Remove Jumps Optimzation for all blocks *)
