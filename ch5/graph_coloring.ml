...
module Make (Elt   : OrderedTypeS)
  ...
  struct
    ...
    let _print_nodemap msg (map : nodemap) =
      Printf.printf "\n%s:\n%s\n\n%!"
    ...
    let saturate_neighbors_with_priority (c : int) (neighbors : elt list)
      ...
      List.fold_left 
        (fun ( (map' : nodemap), (pq' : pqueue) ) (nb : elt) -> 
          ...
          let map'' = EMap.add nb 
            {color=prev.color; saturation=new_saturation} map' in
          let pq'' = PQ.update nb (IntSet.cardinal new_saturation) pq' in
          (map'', pq'')) (map, pq) neighbors
