...
let uncover_live_in_block
  ...
  let compute_live_before_instr (live_sets : LocSet.t list) (instr : X.instr)
    ...
    let before = match instr with 
      | X.JmpIf (_, lbl) -> 
        let after' = LocSet.union (find_label lbl label_map) after
        in add_to_locset rflags_reg after'
      | X.Jmp lbl -> find_label lbl label_map
      | X.Addq (r1, r2)
      | X.Subq (r1, r2) 
      | X.Xorq (r1, r2) -> 
 ...
 *   - Use the liveness map to get the old live-before set for instructions.
