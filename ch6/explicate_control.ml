...
let convert_exp (e : L.exp) : exp =
  match e with
    ...
    | L.Prim (op, es) -> Prim (op, List.map convert_atom es)
...
let rec explicate_assign (e : L.exp) (v : var) (tl : tail) : tail =
  match e with
    ...
    | L.VecRef (a, i) -> Seq (Assign (v, VecRef (convert_atom a, i)), tl)
    | L.VecSet (a1, i, a2) -> Seq (Assign (v, VecSet (convert_atom a1, i, convert_atom a2)), tl)
...
and explicate_pred (e : L.exp) (then_tl : tail) (else_tl : tail) : tail =
  match e with
    ...
    | L.VecRef _ ->
      let temp = !fresh ~base: "vecref" ~sep:"_" in
      explicate_assign e temp (explicate_pred (L.Atm (L.Var temp)) then_tl else_tl)
...
and explicate_tail (e : L.exp) : tail =
  match e with
    ...
    | L.Prim _ 
    | L.Allocate _ 
    | L.GlobalVal _ -> Return (convert_exp e)
    | L.Let (v, e1, e2) ->
    ...
    | L.Collect _ -> explicate_effect e (Return (Atm Void))
    | L.VecLen a -> Return (VecLen (convert_atom a))
    | L.VecRef (a, i) -> Return (VecRef (convert_atom a, i))
    | L.VecSet (a1, i, a2) -> Return (VecSet (convert_atom a1, i, convert_atom a2))
