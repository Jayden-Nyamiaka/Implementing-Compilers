...
let rec ltup_exp_of_sexp = function
  ...
  | List [Atom "vector-ref"; e1; Atom s] ->
    begin
      match int_of_string_opt s with
        | Some i -> VecRef (ltup_exp_of_sexp e1, i)
        | None -> 
          failwithf
  ...
  | List [Atom "vector-set!"; e1; Atom s; e2] ->
    begin
      match int_of_string_opt s with
        | Some i -> VecSet (ltup_exp_of_sexp e1, i, ltup_exp_of_sexp e2)
        | None -> 
            failwithf
  ...
  | List (Atom a :: lst) ->
    begin
      match List.assoc_opt a ops with
        | Some op -> Prim (op, List.map ltup_exp_of_sexp lst)
        | None -> 
