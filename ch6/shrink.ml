...
let rec shrink_exp (e : L.exp) : exp =
  match e with
    ...
    | L.Vec (es, Some ty) -> Vec (List.map shrink_exp es, ty)
    | L.VecLen e -> VecLen (shrink_exp e)
    | L.VecRef (e, i) -> VecRef (shrink_exp e, i)
    | L.VecSet (e1, i, e2) -> VecSet (shrink_exp e1, i, shrink_exp e2)
    | _ -> failwith "shrink_exp: unwanted form"
