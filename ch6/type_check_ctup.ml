          ...
          match vec_ty with
            ...
            | Some (Vector vec) ->
              if i < 0 || i >= Array.length vec then
                ty_err_simple @@
                  Printf.sprintf
                    "vector-ref statement: index %d is out of range" i
              else
                Some vec.(i)
            | Some t ->
              ty_err_simple @@
                Printf.sprintf
                  "vector-ref statement: expected a vector, got: %s"
                  (string_of_ty t)
        end
          ...
          match vec_ty with
            ...
            | Some (Vector vec) ->
              if i < 0 || i >= Array.length vec then
                ty_err_simple @@
                  Printf.sprintf
                    "vector-set! statement: index %d is out of range" i
              else
                begin
            ...
            | Some t ->
              ty_err_simple @@
                Printf.sprintf
                  "vector-set! statement: expected a vector, got: %s"
                  (string_of_ty t)
        end
